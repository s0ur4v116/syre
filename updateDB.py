...
os.chdir("challenges")
startIndex = {"crypto": "120000", "web":"220000", "rev":"320000", "pwn":"420000", "gskills":"520000", "forensics":"620000"}
...
for directory in directories:
	...
	for difficulty in difficulties:
		...
		if len(os.listdir(difficultyPath)) == 0:
			...
			continue
		if not allchalls : maxid = startIndex[directory]
		elif not maxid : maxid = str(int(max(allchalls))+1)
		...
		for challenge in _challenges:
			...
			if challenge not in presentChalls:
				...
				if not os.path.isfile(os.path.join(challengePath, "description.txt")):
					print(f"not found description.txt for the challenge {challenge} under {directory}")
					print("Skipping")
					continue
				if not os.path.isfile(os.path.join(challengePath, "flag.txt")):
					print(f"Not found flag.txt for the challenge {challenge} under {directory}")
					print("skipping")
					continue
				addthispath = "challenges"
				addthispath = os.path.join(addthispath, challengePath)
				toAdd = dict(_id=str(maxid), category=directory, path=addthispath, 
				flag=open(os.path.join(challengePath, "flag.txt")).read().strip(), difficulty=difficulty, name=challenge)
