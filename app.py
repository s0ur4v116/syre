...
class Pager(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        ...
        desc += "\n```"
        embed = Embed(
            title="Here you go!", description=desc, color=nextcord.Color.blurple()
        )
        return embed
...
async def ping(interaction: Interaction):
    await interaction.response.send_message(
        f"Pong! In {round(bot.latency * 1000)}ms.", ephemeral=EPHEMERAL
    )


@bot.slash_command(
    name="about",
    description="In case you want to know more about our lonely Syre!",
    guild_ids=GID,
)
...
@bot.slash_command(
    name="register", description="Register yourself to get Started!", guild_ids=GID
)
async def register(interaction: Interaction):
    ...
    if response == 0:
        ...
        await interaction.response.send_message(
            embed=ON_REGISTRATION_FAILURE_EMBED, ephemeral=EPHEMERAL
        )


@bot.slash_command(
    name="list_challenges",
    description="Take a look of available challenges.",
    guild_ids=GID,
)
async def listChallenges(interaction: Interaction, category: str = CATEGORY_SELECTION):
    ...
    if challList:
        pages = menus.ButtonMenuPages(source=Pager(challList))
...
@bot.slash_command(
    name="start_challenge", description="Start your challenge!", guild_ids=GID
)
async def startChallenge(interaction: Interaction, challengeid: str):
    ...
    if not database.challExists(challengeid):
        await interaction.followup.send(
            embed=CHALL_NOT_FOUND_EMBED, ephemeral=EPHEMERAL
        )
        return

    if database.isChallRunning(uid=user.id, challid=challengeid):
        await interaction.followup.send(
            embed=CHALL_ALREADY_RUNNING_EMBED, ephemeral=EPHEMERAL
        )
        return

    if (
        len(database.userDetails(uid=user.id).get("active_challs"))
        >= MAX_ACTIVE_CHALLENGES
    ):
        await interaction.followup.send(
            embed=MAX_ACTIVE_CHALLENGES, ephemeral=EPHEMERAL
        )
        return
    ...
    if database.getChallCategory(challengeid) in ["pwn", "rev"]:
        if len(docker.botContainersList()) >= MAX_CONTAINERS_COUNT:
            await interaction.followup.send(
                embed=MAX_CONTAINERS_ERROR_EMBED, ephemeral=EPHEMERAL
            )
            return

        if (
            len(database.userDetails(uid=user.id).get("active_containers"))
            >= MAX_CONTAINERS_COUNT_PER_USER
        ):
            await interaction.followup.send(
                embed=MAX_CONTAINERS_PER_USER_ERROR_EMBED, ephemeral=EPHEMERAL
            )
            return

    response: Dict[str, str | List] = database.startChallenge(
        uid=user.id, challid=challengeid
    )
    if response["started"] is False:
        embed: nextcord.Embed = Embed(
            title="Oops!", description="Error occurred:-\n" + response["notes"]
        )
    ...
    embed: nextcord.Embed = Embed(
        title="Running!",
        description="Your challenge has started! :white_check_mark:\n"
        + response["notes"],
    )
    await interaction.followup.send(
        embed=embed,
        files=[File(file) for file in response["files"]],
        ephemeral=EPHEMERAL,
    )


@bot.slash_command(
    name="stop_challenge", description="Stop a running challenge", guild_ids=GID
)
async def stopChallenge(interaction: Interaction, challengeid: str):
    ...
    if not database.isChallRunning(uid=user.id, challid=challengeid):
        await interaction.followup.send(
            embed=CHALL_NOT_STARTED_EMBED, ephemeral=EPHEMERAL
        )
        return
...
@bot.slash_command(
    ...
    description="Check your running challenges.",
    guild_ids=GID,
)
async def activeChallenges(interaction: Interaction):
    ...
    activeChalls: Optional[List[str]] = database.getActiveChallenges(
        interaction.user.id
    )
    embed: nextcord.Embed = Embed(
...
@bot.slash_command(
    name="check_progress", description="Check your progress.", guild_ids=GID
)
async def checkProgress(interaction: Interaction, category: str = CATEGORY_SELECTION):
    ...
    progress_dict: Dict[str, List] = database.getUserStatus(
        uid=user.id, category=category
    )
    if not progress_dict:
        await interaction.followup.send(
            embed=NO_PROGRESS_ERROR_EMBED, ephemeral=EPHEMERAL
        )
        return
    ...
    embed: nextcord.Embed = Embed(
        color=0x5BE61C, title=category.title(), description=desc
    )
    await interaction.followup.send(embed=embed, ephemeral=EPHEMERAL)
...
async def submit_flag(interaction: Interaction, challengeid: str, flag: str):
    ...
    message: nextcord.Message = await interaction.followup.send(
        embed=CHALL_STATUS_CHECK_EMBED, ephemeral=EPHEMERAL
    )
    ...
    if database.checkFlag(uid=user.id, challid=challengeid, flag=flag):
        ...
        await checkCompletionAssignRole(
            userid=user.id, category=database.getChallCategory(challid=challengeid)
        )
    else:
...
async def ephemeral(ctx, action: str | None = None):
    if action is None:
        ...
        reply = (
            "Ephemeral messages are now on"
            if EPHEMERAL
            else "Ephemeral messages are now off"
        )
    else:
        if action.lower() == "on":
            if EPHEMERAL:
...
async def progress(ctx: commands.context, user: nextcord.User | None = None):
    ...
    for category in CHOICES:
        if len(user_info[category]) != 0:
            desc += (
                f"**Challs completed in {category}**\n- "
...
@bot.group(
    name="containers", description="Manage Containers!", invoke_without_command=True
)
@commands.has_any_role(*ADMIN_ROLES)
...
async def count(ctx):
    ...
    if runningContainers == 0:
        await ctx.send("No containers running.")
    elif runningContainers == 1:
        await ctx.send("1 container is running")
    else:
        await ctx.send(f"{runningContainers} container are running.")
...
async def list(ctx: commands.Context, user: nextcord.User | None = None):
    ...
    for i in getContainers:
        ...
        for _, containerid in zip(
            i["active_containers"].keys(), i["active_containers"].values()
        ):
...
async def scoreboard(interaction: Interaction, category=CATEGORY_SELECTION):
    ...
    await interaction.response.send_message(
        embed=Embed(title="LE SALE SCOREBOARD!", description=desc)
    )
